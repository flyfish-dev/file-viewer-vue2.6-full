...
完整格式能力的 Vue 2.6 集成包，已内置 `@file-viewer/preset-all`，无需重复安装 preset。要覆盖所有 Worker / WASM 格式，请按下方“Full 包快速开始”发布 `<部署基址>/file-viewer/` 运行时资源。
...
`@file-viewer/vue2.6-full` 已内置 `@file-viewer/preset-all` 并默认启用完整 renderer 矩阵。不要再安装或传入 `preset-office`、`preset-all` 或单独 renderer。
...
`@file-viewer/vue2.6-full` 默认把 PDF、DOCX、PPT、PPTX、Excel、CAD、Typst、Draw.io、SQLite 和 Archive 资产指向部署基址下的 `file-viewer/`（根部署即 `/file-viewer/`）。运行随包安装的同版本 `file-viewer-copy-assets` 并发布其产物后，不需要逐项手写资源 URL。
